...
// ============================================================
//  ROUTES FIRESTORE — Chantiers
// ============================================================

// GET /chantiers — lister tous les chantiers
app.get('/chantiers', async function(req, res) {
  try {
    var liste = await firestore.listerChantiers();
    res.json({ ok: true, chantiers: liste });
  } catch(err) {
    res.status(500).json({ erreur: err.message });
  }
});

// GET /chantiers/:id — charger un chantier
app.get('/chantiers/:id', async function(req, res) {
  try {
    var data = await firestore.chargerChantier(req.params.id);
    if (!data) return res.status(404).json({ erreur: 'Chantier introuvable' });
    res.json({ ok: true, chantier: data });
  } catch(err) {
    res.status(500).json({ erreur: err.message });
  }
});

// POST /chantiers — sauvegarder un chantier
app.post('/chantiers', async function(req, res) {
  var data = req.body;
  if (!data || !data.numeroDossier) {
    return res.status(400).json({ erreur: 'numeroDossier manquant' });
  }
  try {
    var id = await firestore.sauvegarderChantier(data);
    // Sync Calendar si dates disponibles
    if (data.planning && auth.isAuthenticated) {
      try { await calendar.syncChantier(data); } catch(e) { console.log('Calendar sync:', e.message); }
    }
    res.json({ ok: true, id });
  } catch(err) {
    res.status(500).json({ erreur: err.message });
...
// DELETE /chantiers/:id — supprimer un chantier
app.delete('/chantiers/:id', async function(req, res) {
  try {
    await firestore.supprimerChantier(req.params.id);
    res.json({ ok: true });
  } catch(err) {
    res.status(500).json({ erreur: err.message });
  }
